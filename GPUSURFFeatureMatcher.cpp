	...
	imgpts(imgpts_),use_ratio_test(true)
{
    ...
    _detector = FeatureDetector::create("PyramidORB");
    _extractor = DescriptorExtractor::create("FREAK");
	
    std::cout << " -------------------- extract feature points for all images (Orb + FREAK) -------------------\n";
	
	imgpts.resize(imgs_.size());
	descriptors.resize(imgs_.size());


    _detector->detect(imgs_, imgpts);
    _extractor->compute(imgs_,imgpts,descriptors);
